...
DEFAULT_CSV = "strokes_csv/raw_mouse_cleaned.csv"
DEFAULT_OUTPUT_DIR = "strokes_original"
...
def write_start_end_csv(sampled_strokes: list[tuple[int, pd.DataFrame]], output_csv: Path) -> None:
    ...
    with output_csv.open("w", newline="", encoding="utf-8") as fh:
        ...
        for stroke_id, df in sampled_strokes:
            ordered = df.sort_values("seq").reset_index(drop=True)
            if ordered.empty:
                continue
...
def save_stroke_figure(
    ...
    angular_velocity = compute_heading_series(points, t)
...
def _analyze_sampled_strokes(sampled_strokes: list[tuple[int, pd.DataFrame]], output_dir: Path) -> dict[str, object]:
    ...
    for stroke_id, df in sampled_strokes:
        df = df.sort_values("seq").reset_index(drop=True)
        points = df[["x", "y"]].to_numpy(dtype=float)
        timestamps = df["timestamp"].to_numpy(dtype=float) if "timestamp" in df.columns else np.arange(len(points), dtype=float)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot basic mouse movement samples from a CSV file.")
